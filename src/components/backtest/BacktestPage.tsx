...
import type { ParsedApiError } from './common';
import { getParsedApiError } from './common';
import { ApiErrorAlert, Card, Badge, EmptyState, Pagination, StatusDot, Tooltip } from './common';
import { useUiLanguage } from './common';
import { formatUiText, type UiLanguage } from './common';
import {
...
import { getMarketPhaseSummaryLabel } from './marketPhase';
import RealStrategyBacktest from './RealStrategyBacktest';
...
const EvaluationBacktestPage: React.FC = () => {
  ...
  // Set page title
  useEffect(() => {
    // Embedded workspace retains the main project's document title.
  }, [text.documentTitle]);
  ...
  useEffect(() => {
    ...
    void init().catch((err) => setPageError(getParsedApiError(err)));
  ...
  return (
    <div className="backtest-workspace min-h-full flex flex-col bg-transparent">
      {/* Header */}
        ...
        <div className="flex max-h-[38vh] flex-col gap-3 overflow-y-auto lg:max-h-none lg:w-60 lg:flex-shrink-0">
          {isLoadingPerf ? (
            ...
            <EmptyState
              ...
              className="min-h-[12rem] border-dashed bg-card/45 shadow-none"
            />
          )}
...
const BacktestPage: React.FC = () => {
  const [mode, setMode] = useState<'strategy' | 'evaluation'>('strategy');
  return <div className="backtest-root">
    <nav className="backtest-mode-switch" aria-label="回测模式">
      <button className={mode === 'strategy' ? 'active' : ''} onClick={() => setMode('strategy')}><span>策略回测</span><small>真实行情撮合</small></button>
      <button className={mode === 'evaluation' ? 'active' : ''} onClick={() => setMode('evaluation')}><span>研判验证</span><small>历史 AI 命中率</small></button>
    </nav>
    <div className="backtest-mode-content">{mode === 'strategy' ? <RealStrategyBacktest /> : <EvaluationBacktestPage />}</div>
  </div>;
};
