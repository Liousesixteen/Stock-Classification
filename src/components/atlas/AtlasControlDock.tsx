...
export function AtlasControlDock({
  ...
  return (
    <div className={`atlas-control-dock ${dragOrigin ? "is-dragging" : ""}`} style={style}>
      ...
      <div className="atlas-control-main"><AtlasNodeFinder nodes={nodes} onSelect={onSelectNode} embedded /><div className="atlas-control-filters"><span><Boxes aria-hidden="true" />{focus ? "局部立体" : "产业链全景"}</span><select aria-label="证据信号筛选" value={signalFilter} onChange={(event) => onSignalFilterChange(event.target.value as IndustryGraphSignalFilter)}><option value="all">全部关系</option><option value="upstream">上游输入</option><option value="downstream">下游输出</option><option value="verified">已证实</option><option value="review">待复核</option><option value="missingEvidence">缺证据</option><option value="watchlist">重点跟踪</option></select><b>{stats.categoryCount} 节点 · {stats.companyCount} 公司 · {stats.evidenceCount} 证据</b><button type="button" title="复制当前图谱链接" onClick={() => navigator.clipboard?.writeText(window.location.href)}><Share2 aria-hidden="true" /></button></div></div>
