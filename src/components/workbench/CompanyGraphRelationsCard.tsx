...
export function CompanyGraphRelationsCard({ stockCode, relations, onSaved }: { stockCode: string; relations: CompanyGraphEntityRelation[]; onSaved: () => Promise<void> | void }) {
  ...
  return <section className="detail-section company-graph-relations">
    ...
    {isEditing ? <form onSubmit={submit} className="graph-relation-form">
      ...
      <div className="flex items-center justify-end gap-2">{error ? <span role="alert" className="text-xs text-rose-600">{error}</span> : null}<button type="submit" disabled={saving} className="action-button-primary inline-flex h-8 items-center gap-1.5 px-3 text-xs font-semibold"><Check className="h-3.5 w-3.5" />{saving ? "保存中" : "保存关系"}</button></div>
    </form> : null}
