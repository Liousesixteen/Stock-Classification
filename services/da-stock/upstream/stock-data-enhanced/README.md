...
> Enhanced fork in this repository: this copy keeps the original A-share 7-layer skill and adds `scripts/provider_cli.py` for the extra providers from `data-sources`, including HK/US data, search providers, social sentiment, AlphaSift, and LLM/Vision readiness checks. Use `make install` in this directory to install the enhanced dependency set.
