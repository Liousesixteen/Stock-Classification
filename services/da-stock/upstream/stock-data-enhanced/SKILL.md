---
name: a-stock-data-enhanced
description: Use when retrieving or configuring stock market data across A-share, Hong Kong, and US markets, including quotes, K-line history, fundamentals, market review, research/news search, social sentiment, AlphaSift screening, LLM/Vision provider readiness, provider fallback, API keys, and data-source troubleshooting.
origin: custom
version: 4.0-enhanced
---

> 本目录是从原 `a-stock-data` skill 复制出的增强版；原始 skill 不在此目录内修改。
> 增强版保留原 A 股七层直连数据能力，并额外集成 `data-sources` 中的所有跨市场 provider、配置 key 和统一 CLI。

# A股/港股/美股全栈数据工具包 Enhanced

## Enhanced Provider CLI

新增统一入口：`scripts/provider_cli.py`。它覆盖 `data-sources` 中列出的全部 provider 元数据，并为通用可实现的数据源提供直接抓取命令。

```bash
# 列出全部 provider 元数据
bin/run_skill scripts/provider_cli.py providers

# 离线检查 .env 中哪些 provider 已配置，不打印密钥值
bin/run_skill scripts/provider_cli.py --env /path/to/.env check-config

# 实时行情
bin/run_skill scripts/provider_cli.py --env /path/to/.env quote 600519 --market cn --provider auto
bin/run_skill scripts/provider_cli.py --env /path/to/.env quote AAPL --market us --provider yfinance
bin/run_skill scripts/provider_cli.py --env /path/to/.env quote HK00700 --market hk --provider longbridge

# 历史 K 线
bin/run_skill scripts/provider_cli.py --env /path/to/.env history AAPL --market us --provider auto --days 30
bin/run_skill scripts/provider_cli.py --env /path/to/.env history 600519 --market cn --provider akshare --days 30

# 新闻搜索
bin/run_skill scripts/provider_cli.py --env /path/to/.env news "贵州茅台 业绩" --provider auto --limit 5 --days 7

# 社交情绪与股票索引
bin/run_skill scripts/provider_cli.py --env /path/to/.env social TSLA
bin/run_skill scripts/provider_cli.py --env /path/to/.env stock-index --limit 5
```
...
| Area | Providers |
| --- | --- |
| 原 A 股直连能力 | `mootdx`, `腾讯财经`, `百度股市通`, `东财 reportapi/datacenter/push2/push2his/search-api-web/np-weblist`, `同花顺 THS/basic/hsgtApi`, `iwencai`, `财联社`, `新浪财报三表`, `巨潮 cninfo` |
| A 股通用/SDK 数据源 | `efinance`, `akshare`, `tushare`, `pytdx`, `baostock`, `tickflow`, `stock-index-remote`, `alphasift` |
| 港股/美股/全球 | `longbridge`, `yfinance`, `stooq`, `finnhub`, `alphavantage` |
| 新闻/搜索 | `anspire-search`, `bocha`, `tavily`, `serpapi`, `brave`, `minimax-search`, `searxng` |
| 情绪 | `social-sentiment` |
| LLM/Vision readiness | `llm-anspire`, `llm-aihubmix`, `llm-gemini`, `llm-deepseek`, `llm-anthropic`, `llm-openai`, `llm-moonshot`, `llm-dashscope`, `llm-zhipu`, `llm-minimax`, `llm-mimo`, `llm-volcengine`, `llm-siliconflow`, `llm-openrouter`, `llm-ollama`, `vision` |

### What The CLI Implements Directly

- `quote`: `tencent`, `sina`, `efinance`, `akshare`, `yfinance`, `finnhub`, `alphavantage`, `longbridge`
- `history`: `yfinance`, `stooq`, `alphavantage`, `akshare`, `efinance`, `tushare`, `baostock`
- `news`: `anspire-search`, `bocha`, `tavily`, `brave`, `serpapi`, `minimax-search`, `searxng`
- `social`: `social-sentiment`
- `stock-index`: remote stock index JSON
- `check-config`: every provider group, including LLM/Vision, AlphaSift, Longbridge, TickFlow, and all search providers

Providers that require account-specific SDK state or a target application runtime are still represented in metadata and config checks even when live fetching is delegated to their SDK/app wiring.
