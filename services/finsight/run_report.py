...
async def run_report(
    ...
    Args:
        ...
        config_file_path: YAML configuration to load.
        max_concurrent: Maximum number of concurrent tasks. If None, uses MAX_CONCURRENT env var or unlimited.
        max_iterations: Maximum LLM/code iterations for each agent stage.
        generate_tasks: Whether to let the LLM add tasks beyond the configured task lists.
        enable_chart: Whether analyzers and the report generator should create charts.
        add_reference_section: Whether to append the report reference section.
        echo: Whether to log every LLM response and action result.
    """
    ...
    if generate_tasks and not memory.generated_collect_tasks:
        ...
        logger.info("Automatic collect-task generation disabled")
    
    # Generate analysis tasks if not already generated
    if generate_tasks and not memory.generated_analysis_tasks:
        ...
        logger.info("Automatic analysis-task generation disabled")
    ...
    tasks_to_run.append({
        ...
        'task_input': {
            'input_data': {
                'task': f'Research target: {config.config["target_name"]} (ticker: {config.config["stock_code"]})',
                'task_type': config.config.get('target_type', 'general'),
            ...
            'resume': resume,
            'enable_chart': enable_chart,
            'add_reference_section': add_reference_section,
        },
        'agent_kwargs': {
            'use_llm_name': use_llm_name,
            'use_embedding_name': use_embedding_name,
        ...
        if async_tasks:
            ...
            for agent_info, result in zip(tasks_to_run, results):
                ...
                if isinstance(result, Exception):
                    ...
                    task_failures.append((agent.id, result))
                else:
    ...
    memory.save()
    if task_failures:
        failed_ids = ", ".join(agent_id for agent_id, _ in task_failures)
        raise RuntimeError(f"{len(task_failures)} task(s) failed: {failed_ids}")
    evidence_graph_result = memory.finalize_evidence_graph()
    logger.info(f"Evidence graph finalized: {evidence_graph_result.get('health', evidence_graph_result)}")
    logger.info("All tasks completed")


def parse_args():
    parser = argparse.ArgumentParser(description="Run the FinSight research pipeline.")
    parser.add_argument("--config", default="my_config.yaml", help="Path to the YAML run configuration.")
    parser.add_argument("--max-concurrent", type=int, default=MAX_CONCURRENT)
    parser.add_argument("--max-iterations", type=int, default=20)
    parser.add_argument("--resume", action=argparse.BooleanOptionalAction, default=IF_RESUME)
    parser.add_argument("--generate-tasks", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--charts", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--references", action=argparse.BooleanOptionalAction, default=True)
...
if __name__ == '__main__':
    ...
    asyncio.run(run_report(
        ...
        enable_chart=args.charts,
        add_reference_section=args.references,
        echo=args.echo,
    ))
