...
class ReportGenerator(BaseAgent):
    ...
    async def _prepare_executor(self):
        ...
        self.code_executor.set_variable("build_claim_material_pack", self.memory.build_evidence_material_pack)
        self.code_executor.set_variable("register_claim", self.memory.register_claim)
        self.code_executor.set_variable("bind_evidence", self.memory.bind_evidence)
        self.code_executor.set_variable("audit_claim", self.memory.audit_claim)
        self.code_executor.set_variable("get_evidence_repair_plan", self.memory.get_evidence_repair_plan)
...
- build_claim_material_pack(query, limit=12): retrieve ranked, source-grounded material before writing a claim.
- register_claim(text, section='', evidence_ids=[], fact_ids=[], calculation_ids=[], chart_ids=[]): create a Claim Ledger entry before final output.
- bind_evidence(claim_id, evidence_ids): add candidate support without inventing citations.
- audit_claim(claim_id): verify traceability, authority, stale dependencies, and unresolved conflicts.
- get_evidence_repair_plan(): list only the calculations, charts, claims, and sections requiring incremental repair.
Every material quantitative or causal statement should be registered and evidence-bound.
"""
        
        # Prepare data information for the agent
        collect_data_list = self.memory.get_collect_data(exclude_type=['search', 'click'])
        analysis_result_list = self.memory.get_analysis_result()
        data_info = self.memory.build_evidence_material_pack(
            f"{task}\n{section_outline or ''}",
            limit=int(self.config.config.get('evidence_graph', {}).get('writing_material_limit', 12)),
    ...
    async def _prepare_outline_prompt(self, input_data):
        ...
        data_info = self.memory.build_evidence_material_pack(
            input_data['task'],
            limit=int(self.config.config.get('evidence_graph', {}).get('outline_material_limit', 16)),
            max_chars=int(self.config.config.get('evidence_graph', {}).get('material_pack_max_chars', 12000)),
        )
    ...
    async def async_run(
        ...
        if self._phase == 'sections':
            ...
            for idx, section in enumerate(report.sections):
                ...
                claim_ids = self.memory.register_report_section(section.title, final_section)
                self.logger.info(f"[EvidenceGraph] Registered {len(claim_ids)} claims for section: {section.title}")
