...
class DataAnalyzer(BaseAgent):
    ...
    async def _prepare_executor(self):
        ...
        self.code_executor.set_variable("query_analysis_bundle", self.memory.get_evidence_bundle)
        self.code_executor.set_variable("register_financial_fact", self.memory.register_financial_fact)
        self.code_executor.set_variable("register_calculation", self.memory.register_calculation)
        self.code_executor.set_variable("register_chart", self.memory.register_chart)
        self.code_executor.set_variable("mine_implicit_relationships", self.memory.mine_implicit_relationships)
    ...
    async def _prepare_init_prompt(self, input_data: dict) -> list[dict]:
        ...
        graph_api_description = self.DATA_API_PROMPT + """

Evidence graph APIs available in code:
- query_analysis_bundle(query, limit=12): retrieve task-conditioned evidence, facts, calculations, and provenance edges.
- register_financial_fact(metric_name, value, period, unit='', currency=None, evidence_ids=[]): persist a normalized fact and return its fact ID.
- register_calculation(name, formula, value, input_fact_ids, unit='', code='', parameters={}): persist reproducible calculation lineage.
- register_chart(name, path, data_node_ids, code='', description='', review_status='unreviewed'): bind a generated chart to exact facts/calculations.
- mine_implicit_relationships(metric_id=None): create candidate trend hypotheses from comparable temporal facts; hypotheses still require evidence-based interpretation.
Use graph IDs instead of relying only on prose whenever a quantitative conclusion or chart is created.
"""

        if enable_chart:
            prompt = self.DATA_ANALYSIS_PROMPT.format(
                api_descriptions=graph_api_description,
                data_info=data_info,
    ...
    async def _format_collect_data(self, analysis_task, collect_data_list):
        ...
        formatted_data = self.memory.build_evidence_material_pack(
            analysis_task,
            limit=int(self.config.config.get('evidence_graph', {}).get('analysis_material_limit', 12)),
            max_chars=int(self.config.config.get('evidence_graph', {}).get('material_pack_max_chars', 12000)),
        )
        formatted_data += "\n\n"
        for idx,item in enumerate(collect_data_list):
            formatted_data += f"Data (id:{idx}):\n{item.brief_str()}\n\n"
