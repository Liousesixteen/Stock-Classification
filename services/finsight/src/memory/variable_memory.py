...
from src.evidence_graph import EvidenceGraphService, TaskSpec
...
class Memory:
    def __init__(
        ...
        # Shared, durable evidence memory used by every pipeline stage.  The
        # graph persists independently from dill checkpoints, so a partial run
        # never loses already collected provenance.
        self.evidence_graph = None
        evidence_graph_config = config.config.get('evidence_graph', {})
        if evidence_graph_config.get('enabled', True):
            try:
                self.evidence_graph = EvidenceGraphService(
                    working_dir=config.working_dir,
                    task_spec=TaskSpec(
                        target_name=config.config.get('target_name', 'unknown'),
                        stock_code=str(config.config.get('stock_code', '')),
                        target_type=config.config.get('target_type', 'general'),
                        market=config.config.get('market', ''),
                        as_of_date=evidence_graph_config.get('as_of_date'),
                        required_metrics=list(evidence_graph_config.get('required_metrics', [])),
                        modalities=list(evidence_graph_config.get('modalities', ['text', 'table', 'image', 'timeseries', 'code'])),
                        max_nodes=int(evidence_graph_config.get('max_nodes', 500)),
                    ),
                    authority_threshold=float(evidence_graph_config.get('authority_threshold', 0.65)),
                    auto_extract_facts=bool(evidence_graph_config.get('auto_extract_facts', True)),
                    max_auto_facts=int(evidence_graph_config.get('max_auto_facts', 500)),
                )
            except Exception as exc:
                if evidence_graph_config.get('strict', False):
                    raise
                self.logger.error(f"Evidence graph initialization failed: {exc}", exc_info=True)
    ...
    def add_data(self, data: Any):
        ...
        if self.evidence_graph is not None:
            try:
                self.evidence_graph.ingest_runtime_object(data)
            except Exception as exc:
                self.logger.error(f"Evidence graph ingestion failed for {type(data).__name__}: {exc}", exc_info=True)
        return True

    def get_evidence_bundle(self, query: str, limit: int = 12) -> dict:
        if self.evidence_graph is None:
            return {"query": query, "results": [], "context_node_ids": [], "context_edges": []}
        return self.evidence_graph.query(query, limit=limit)

    def build_evidence_material_pack(self, query: str, limit: int = 12, max_chars: int = 12000) -> str:
        if self.evidence_graph is None:
            return "Evidence graph is disabled."
        return self.evidence_graph.build_material_pack(query, limit=limit, max_chars=max_chars)

    def register_financial_fact(self, **kwargs):
        if self.evidence_graph is None:
            raise RuntimeError("Evidence graph is disabled")
        delta = self.evidence_graph.register_financial_fact(**kwargs)
        fact_ids = [
            node_id
            for node_id in delta.added_nodes + delta.updated_nodes + delta.unchanged_nodes
            if node_id.startswith('fact_')
        ]
        return fact_ids[0] if fact_ids else None

    def register_calculation(self, **kwargs):
        if self.evidence_graph is None:
            raise RuntimeError("Evidence graph is disabled")
        return self.evidence_graph.register_calculation(**kwargs)

    def register_chart(self, **kwargs):
        if self.evidence_graph is None:
            raise RuntimeError("Evidence graph is disabled")
        return self.evidence_graph.register_chart(**kwargs)

    def mine_implicit_relationships(self, metric_id: str = None):
        if self.evidence_graph is None:
            return []
        return self.evidence_graph.mine_implicit_relationships(metric_id)

    def register_claim(self, **kwargs):
        if self.evidence_graph is None:
            raise RuntimeError("Evidence graph is disabled")
        return self.evidence_graph.register_claim(**kwargs)

    def bind_evidence(self, claim_id: str, evidence_ids):
        if self.evidence_graph is None:
            raise RuntimeError("Evidence graph is disabled")
        return self.evidence_graph.bind_evidence(claim_id, evidence_ids).to_dict()

    def audit_claim(self, claim_id: str) -> dict:
        if self.evidence_graph is None:
            raise RuntimeError("Evidence graph is disabled")
        return self.evidence_graph.audit_claim(claim_id).to_dict()

    def get_evidence_repair_plan(self):
        if self.evidence_graph is None:
            return []
        return self.evidence_graph.build_repair_plan()

    def register_report_section(self, title: str, content: str) -> list[str]:
        if self.evidence_graph is None:
            return []
        return self.evidence_graph.register_report_section(title, content)

    def finalize_evidence_graph(self) -> dict:
        if self.evidence_graph is None:
            return {"enabled": False}
        audits = [result.to_dict() for result in self.evidence_graph.audit_all_claims()]
        snapshot = self.evidence_graph.export_snapshot()
        return {
            "enabled": True,
            "audits": audits,
            "repair_plan": self.evidence_graph.build_repair_plan(),
            "coverage": self.evidence_graph.coverage_report(),
            "snapshot": snapshot,
            "health": self.evidence_graph.health(),
        }
