#!/usr/bin/env python3
"""Yidianx market desk — standard-library, local-first investment workbench."""
...
UA = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Yidianx/1.2"
...
def is_loopback(handler: BaseHTTPRequestHandler) -> bool:
    try:
        direct_loopback = handler.client_address[0] in {"127.0.0.1", "::1"}
    except Exception:
        return False
    if handler.headers.get("X-Forwarded-For") or handler.headers.get("X-Real-IP"):
        return direct_loopback and os.environ.get("RICH_TRUST_LOOPBACK_PROXY") == "true"
    return direct_loopback


class Handler(BaseHTTPRequestHandler):
    server_version = "Yidianx/1.2"
    ...
    def do_GET(self) -> None:
        ...
        if path.startswith("/api/") and not self._authorized():
            self.send_response(401); self.send_header("WWW-Authenticate", 'Basic realm="Yidianx"')
...
def main() -> None:
    cfg = config()
    host = os.environ.get("RICH_WORKBENCH_HOST", cfg.get("监听地址", "127.0.0.1"))
    port = int(os.environ.get("RICH_WORKBENCH_PORT", cfg.get("端口", 8765)))
    ...
    print(f"Yidianx market service running at http://127.0.0.1:{port}")
